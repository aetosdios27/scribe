...
2. When a verified version pull request has been merged and no unreleased fragments remain, the release job reuses the eight native executables the verified CI run already built from that commit, downloading those immutable artifacts instead of rebuilding, then packs and inspects the four public packages (`@scribe-sdk/cli` bundles all eight native binaries) and runs Scribe's guarded publisher. Every package uses an explicit `--tag beta`; existing versions are skipped; every `beta` dist-tag must converge.
