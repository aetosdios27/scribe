...
import { normalizeRepositoryPath } from "./lib/platform.mjs";
...
async function repositoryFiles(directory) {
  ...
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    if ([".git", ".next", ".scribe-local", ".scribe-pack", ".scribe-release", "coverage", "dist", "node_modules", "out", "playwright-report", "target", "test-results"].includes(entry.name)) continue;
