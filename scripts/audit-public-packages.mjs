...
export const publicPackages = Object.freeze(["styles", "react", "mdx", "cli"]);
const commandTimeoutMilliseconds = 120_000;
const acceptedAdvisories = new Map([
  [1124022, "Monaco 0.56.0 pins DOMPurify 3.4.8; Scribe does not enable custom-element sanitizer hooks."],
  [1124233, "Monaco 0.56.0 pins DOMPurify 3.4.8; Scribe does not expose DOMPurify setConfig or mutable sanitizer policy."],
  [1124234, "Monaco 0.56.0 pins DOMPurify 3.4.8; Scribe does not use DOMPurify Trusted Types configuration."]
]);
...
export async function auditPublicPackages() {
  ...
  for (const directory of publicPackages) {
    ...
    for (const group of [manifest.dependencies ?? {}, manifest.peerDependencies ?? {}]) {
      for (const [name, version] of Object.entries(group)) {
        if (name.startsWith("@scribe-sdk/")) continue;
  ...
  const auditDirectory = await mkdtemp(join(tmpdir(), "scribe-public-audit-"));
  ...
  try {
    ...
    const installedTree = runJson(auditDirectory, "npm", ["ls", "--all", "--json"]);
    ...
    if (!response.ok) {
      throw new Error(`npm bulk advisory request failed with HTTP ${response.status}.`);
    }
    const report = decodeAuditResponse(Buffer.from(await response.arrayBuffer()));
    const advisories = Object.entries(report).flatMap(([name, matches]) => (
      matches.map((advisory) => ({ ...advisory, name }))
    ));
    const blockingAdvisories = advisories.filter(({ id }) => !acceptedAdvisories.has(id));
    for (const advisory of advisories) {
      const accepted = acceptedAdvisories.get(advisory.id);
      if (accepted !== undefined) {
        process.stdout.write(`Accepted ${advisory.severity} advisory ${advisory.id} for ${advisory.name}: ${accepted}\n`);
      }
    }
    if (blockingAdvisories.length > 0) {
      for (const advisory of blockingAdvisories) {
        process.stderr.write(
          `${advisory.severity ?? "unknown"}: ${advisory.name} — ${advisory.title ?? advisory.url ?? "known vulnerability"}\n`
        );
      }
      throw new Error(`Production dependency audit found ${blockingAdvisories.length} unaccepted advisories.`);
    }
...
function collectPackageVersions(tree) {
  ...
  return Object.fromEntries(
    [...versions].map(([name, values]) => [name, [...values].sort()])
  );
}
